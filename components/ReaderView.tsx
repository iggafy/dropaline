    ...
    <div className={`h-full flex flex-col bg-white overflow-hidden relative ${paperSaver ? 'bg-[#fcfcfc]' : 'bg-white'}`}>
      ...
      <header className="h-16 flex items-center justify-between px-8 border-b border-[#f2f2f2] shrink-0">
        ...
        <div className="flex items-center gap-4">
           {/* Dev Tool: Simulate Drop */}
           <button 
             onClick={onSimulateDrop}
             className="text-[10px] font-bold text-[#0066cc] bg-[#f0f7ff] hover:bg-[#e0efff] px-3 py-1 rounded-md transition-colors"
           >
             + SIMULATE DROP
           </button>
           
          <div className="flex items-center gap-2 bg-[#f5f5f7] px-3 py-1.5 rounded-full text-xs font-medium text-[#48484a]">
            <div className={`w-1.5 h-1.5 rounded-full ${printer.isConnected ? 'bg-green-500' : 'bg-red-500'}`}></div>
            {printer.name}
          </div>
        </div>
      </header>
      ...
      <div className="flex-1 overflow-y-auto p-8 space-y-6">
        {drops.map((drop) => (
          <div 
            key={drop.id} 
            className={`group relative border rounded-2xl p-6 shadow-sm hover:shadow-md transition-all duration-300 max-w-2xl mx-auto ${
              paperSaver 
              ...
              <div className="flex items-center gap-3">
                 <button 
                   ...
                   className={`flex items-center gap-1.5 px-3 py-1.5 rounded-full transition-colors text-xs font-medium ${
                     drop.liked ? 'text-red-500 bg-red-50' : 'text-[#86868b] hover:bg-[#f5f5f7]'
                   }`}
                 >
                 ...
                 <button 
                   ...
                   className={`flex items-center gap-1.5 px-3 py-1.5 rounded-full transition-colors text-xs font-medium ${
                     expandedCommentsId === drop.id ? 'bg-[#f5f5f7] text-[#1d1d1f]' : 'text-[#86868b] hover:bg-[#f5f5f7]'
                   }`}
                 >
              ...
              <div className="mt-4 pt-4 border-t border-[#f2f2f2] animate-in fade-in slide-in-from-top-2">
                  ...
                  {drop.commentList && drop.commentList.length > 0 ? (
                    drop.commentList.map((comment) => (
                      <div key={comment.id} className="flex gap-2 items-start">
                        <div className={`w-6 h-6 rounded-full shrink-0 ${paperSaver ? 'bg-gray-300' : 'bg-gray-200'}`}>
                           {/* Avatar placeholder */}
                        </div>
                ...
                <div className="relative">
                  <input 
                    ...
                    className={`w-full border rounded-full py-2 pl-4 pr-10 text-xs focus:outline-none transition-colors ${
                      paperSaver 
        ...
        {drops.length === 0 && (
          <div className="h-full flex flex-col items-center justify-center text-center pt-20">
            ...
            <button 
             onClick={onSimulateDrop}
             className="mt-6 text-xs font-bold text-[#0066cc] bg-[#f0f7ff] hover:bg-[#e0efff] px-4 py-2 rounded-full transition-colors"
           >
             Simulate Incoming Drop
           </button>
          </div>
        )}
      </div>
